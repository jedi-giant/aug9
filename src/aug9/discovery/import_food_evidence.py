...
from aug9.discovery.food_evidence_importer import FoodEvidenceCsvImporter
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Import structured food evidence from an authorised CSV"
    )
    ...
    parser.add_argument(
        "--permission",
        required=True,
        choices=["licensed_partner", "legal_reviewed"],
    )
    ...
    summary = FoodEvidenceCsvImporter(DiscoveryRepository(), source).run(args.file)
    print(
        "Food evidence import complete: "
